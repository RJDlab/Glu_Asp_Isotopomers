...
dfTransitions<-
  read.xlsx("inputAsp.xlsx", sheet = 1, skipEmptyRows = T, skipEmptyCols = T, check.names = F)
...
matrixNForRegressionAnalysis<-function(z){
  ...
  vecPositional<-
    expand.grid(rep(list(0:1), AtomicCompositionParent["C"])) %>% unite(Positional,everything(),sep = "") %>% unlist() 
