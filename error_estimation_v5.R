rm(list=ls());gc()
setwd('/project/CRI/DeBerardinis_lab/lcai/ForOthers/Feng/07202022/')
...
m.list<-m.list[unlist(lapply(m.list,ncol))>10]
...
for(D.name in names(m.list)){
  ...
  test.plot<-function(test.err){
    ggplot(data.table(isotopomer=colnames(D),value=as.numeric(as.matrix(test.err))),aes(x=value,y=isotopomer))+
      geom_point_rast(height=.1,alpha=.1)+geom_boxplot(outlier.shape = NA,alpha=.5)+theme_bw()
  }
